"""
Regime Engine v6 — Contact Zone Continuation
...
def compute_v3_features(df):
    ...
    for date, day_df in df.groupby('date'):
        ...
        for idx in range(len(day_df)):
            ...
            # v6: Compute thesis-direction targets regardless of current king_dir
            # This catches the case where king_dir wobbles BEARISH but thesis is BULLISH
            thesis_targets = 0
            thesis_growth = 0
            if thesis_dir == 'BULLISH':
                if above_abs >= 5_000_000 and above_strike > spot:
                    thesis_targets += 1
                    thesis_growth += max(0, above_growth)
            elif thesis_dir == 'BEARISH':
                if below_abs >= 5_000_000 and below_strike < spot:
                    thesis_targets += 1
                    thesis_growth += max(0, below_growth)

            bullish_migration_active = (
                thesis_dir == 'BULLISH' and
                thesis_migration_count >= 1 and
                (same_dir_targets > 0 or thesis_targets > 0) and
                (same_dir_growth >= 0 or thesis_growth >= 0) and
                takeover_risk < 50
            )
            ...
            # v6: AT_SPOT OR brief BEARISH wobble during active bullish migration
            # Both are "contact zone" — thesis continues
            bullish_contact = (
                thesis_dir == 'BULLISH' and
                bullish_migration_active and
                king_dir in ('AT_SPOT', 'BEARISH') and
                abs(king_dist) < 20  # wobble zone around current king
            )

            # Use thesis-aware targets for continuation check
            has_bullish_continuation = (thesis_targets > 0 or same_dir_targets > 0)

            if bullish_contact and has_bullish_continuation:
                # Thesis alive — price at/near king, higher targets building
                calls_score += at_spot_continuation  # 20-40 pts
                calls_score += min(20, thesis_bars // 5)  # long thesis = more trust
                calls_score += min(15, thesis_migration_count * 8)  # migration maturity bonus
                if same_dir_growth > 0.1:
                    calls_score += 15
                if mom_15 > 0:
                    calls_score += 10
            elif bullish_contact:
                # Migration active but targets not growing right now — still hold
                calls_score += 20  # base continuation
                calls_score += min(15, thesis_bars // 10)  # thesis duration trust
                calls_score += min(10, thesis_migration_count * 5)
            ...
            # v6: AT_SPOT OR brief BULLISH wobble during active bearish migration
            bearish_contact = (
                thesis_dir == 'BEARISH' and
                bearish_migration_active and
                king_dir in ('AT_SPOT', 'BULLISH') and
                abs(king_dist) < 20
            )

            has_bearish_continuation = (thesis_targets > 0 or same_dir_targets > 0)

            if bearish_contact and has_bearish_continuation:
                puts_score += at_spot_continuation
                puts_score += min(20, thesis_bars // 5)
                puts_score += min(15, thesis_migration_count * 8)
                if same_dir_growth > 0.1:
                    puts_score += 15
                if mom_15 < 0:
                    puts_score += 10
            elif bearish_contact:
                puts_score += 20
                puts_score += min(15, thesis_bars // 10)
                puts_score += min(10, thesis_migration_count * 5)
...
if __name__ == '__main__':
    ...
    v3_df.to_csv('research/regime_v6.csv', index=False)

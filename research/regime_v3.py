"""
Regime Engine v3 — King-Persistence-First Framework
    ...
    for date, day_df in df.groupby('date'):
        ...
        for idx in range(len(day_df)):
            ...
            if king_dir != 'AT_SPOT':
                ...
                elif persistence_bars >= 10:
                    # Thesis flip — opposite direction took over with persistence
                    thesis_dir = king_dir
                    thesis_start_frame = frame
                    thesis_king_strikes = {king_strike}
            ...
            if king_dir == 'BULLISH':
                ...
                if price_dipping and targets_still_growing:
                    dip_vs_structure = 30  # dip is temporary, structure strong
                elif price_dipping and not targets_still_growing:
                    dip_vs_structure = -20  # real weakness
                elif not price_dipping and targets_still_growing:
                    dip_vs_structure = 50  # everything aligned
            elif king_dir == 'BEARISH':
                price_dipping = mom_15 > 3  # "dipping" against bearish = bouncing
                targets_still_growing = same_dir_growth > 0.1
                if price_dipping and targets_still_growing:
                    dip_vs_structure = 30
                elif price_dipping and not targets_still_growing:
            ...
            calls_score = 0
            if king_dir == 'BULLISH':
                # King above spot — base bullish signal
                calls_score += 25
                # King persistence: the longer it's been king, the stronger
                calls_score += min(30, persistence_bars * 1.0)
                # King strengthening
                if king_strengthening:
                    calls_score += 15
                # Same-direction targets building higher
                calls_score += min(20, same_dir_targets * 10)
                calls_score += min(15, same_dir_growth * 30)
                # Same-direction migration (6910→6925) = bonus
                if king_migrated_same_dir:
                    calls_score += 15
                # Price momentum confirms
                if mom_15 > 3:
                    calls_score += 10
                # Dip vs structure: hold through pullbacks if structure intact
            ...
            if takeover_risk > 30 and takeover_risk < 70:
                straddle_score += 10  # contested = vol
            if abs(calls_score - puts_score) < 15:
                straddle_score += 10  # close call = uncertainty
            ...
            if 720 <= minute <= 780:
                no_trade_score += 20  # lunch dead zone
...
if __name__ == '__main__':
    ...
    v3_df.to_csv('research/regime_v3.csv', index=False)
